...
router.get('/', async (req, res) => {
    try {
        ...
        const [rows] = await pool.execute(`SELECT p.*, u.username, u.nickname, u.avatar_url as user_avatar,
...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        await pool.execute('INSERT INTO posts (user_id, pet_id, pet_display, content, images, videos, location, show_time, show_location) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [userId, pet_id || null, petDisplay, content.trim(), imgs, vids, location || null, show_time !== false ? 1 : 0, show_location ? 1 : 0]);
        ...
        const pet = petId
            ? (await pool.execute('SELECT name, avatar_url FROM pets WHERE id = ?', [petId]))[0][0]
            : null;
...
router.delete('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        if (created < monthAgo)
            return res.status(403).json({ error: '发布超过一个月后无法删除' });
...
router.post('/:id/like', authMiddleware, async (req, res) => {
    try {
        const [ins] = await pool.execute('INSERT IGNORE INTO likes (user_id, post_id) VALUES (?, ?)', [req.userId, req.params.id]);
        const affected = ins.affectedRows;
        if (affected > 0) {
            await pool.execute('UPDATE pets p INNER JOIN posts po ON p.id = po.pet_id SET p.popularity = p.popularity + 1 WHERE po.id = ?', [req.params.id]);
        }
...
router.delete('/:id/like', authMiddleware, async (req, res) => {
    try {
        ...
        const affected = del.affectedRows;
        if (affected > 0) {
            await pool.execute('UPDATE pets p INNER JOIN posts po ON p.id = po.pet_id SET p.popularity = GREATEST(0, p.popularity - 1) WHERE po.id = ?', [req.params.id]);
        }
        await pool.execute('UPDATE posts SET likes_count = (SELECT COUNT(*) FROM likes WHERE post_id = ?) WHERE id = ?', [req.params.id, req.params.id]);
        const [r] = await pool.execute('SELECT likes_count FROM posts WHERE id = ?', [req.params.id]);
        const likes = r[0];
...
router.get('/:id/comments', optionalAuthMiddleware, async (req, res) => {
    try {
        ...
        const offset = Math.max(0, parseInt(String(req.query.offset), 10) || 0);
        const [rows] = await pool.execute(`SELECT c.*, u.username, u.nickname, u.avatar_url,
        (SELECT COUNT(*) FROM comment_likes WHERE comment_id = c.id) as likes_count
       FROM comments c
       JOIN users u ON c.user_id = u.id
       WHERE c.post_id = ?
       ORDER BY c.created_at
       LIMIT ${limit} OFFSET ${offset}`, [req.params.id]);
        const comments = rows;
        if (userId) {
            const ids = comments.map((c) => c.id).filter(Boolean);
            if (ids.length > 0) {
                const [likedRows] = await pool.execute('SELECT comment_id FROM comment_likes WHERE user_id = ? AND comment_id IN (?)', [userId, ids]);
...
router.post('/:id/comments', authMiddleware, async (req, res) => {
    try {
        ...
        const count = cnt[0]?.comments_count ?? 0;
        if (count >= MAX_COMMENTS_PER_POST)
            return res.status(400).json({ error: '该动态评论已满（最多30条）' });
...
router.post('/:postId/comments/:commentId/like', authMiddleware, async (req, res) => {
    try {
        ...
        const [r] = await pool.execute('SELECT COUNT(*) as cnt FROM comment_likes WHERE comment_id = ?', [commentId]);
        const cnt = r[0]?.cnt ?? 0;
