...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        const [rows] = await pool.execute(`SELECT o.*, JSON_ARRAYAGG(JSON_OBJECT('product_id', oi.product_id, 'quantity', oi.quantity, 'price', oi.price)) as items
       FROM orders o LEFT JOIN order_items oi ON o.id = oi.order_id WHERE o.user_id = ? GROUP BY o.id ORDER BY o.created_at DESC`, [userId]);
        res.json(rows);
    }
    catch {
...
const PERMANENT_OWN_TYPES = ['pet_accessory', 'avatar'];
/** 下单：虚拟币支付，扣款后立即发放商品 */
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        for (const it of items) {
            ...
            const p = r[0];
            if (!p)
                return res.status(400).json({ error: `商品 ${it.product_id} 不存在` });
            if (PERMANENT_OWN_TYPES.includes((p.type || ''))) {
            ...
            if (p.type === 'credit_restore') {
                if (credit >= 1)
                    return res.status(400).json({ error: '信誉分大于0时无需购买恢复' });
                if ((it.quantity || 1) > 1)
                    return res.status(400).json({ error: '信誉分恢复每次仅可购买1分' });
                unitPrice = 100;
            }
        ...
        for (const it of items) {
            ...
            if (prod.type === 'credit_restore') {
                ...
                const isPermanent = PERMANENT_OWN_TYPES.includes((prod.type || ''));
                for (let i = 0; i < qty; i++) {
                    if (isPermanent && i > 0)
                        break;
                ...
                if (prod.type !== 'boost')
                    totalBoost += (prod.popularity_boost || 0) * (isPermanent ? 1 : qty);
            }
        }
...
router.get('/:id/status', authMiddleware, async (req, res) => {
    try {
        ...
        const [rows] = await pool.execute('SELECT id, status, paid_at FROM orders WHERE id = ? AND user_id = ?', [orderId, userId]);
        const o = rows[0];
        if (!o)
...
router.post('/:id/wechat-prepay', authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        const orderId = parseInt(req.params.id, 10);
        if (isNaN(orderId))
            return res.status(400).json({ error: '无效订单' });
        const [rows] = await pool.execute('SELECT id, user_id, total_amount, status FROM orders WHERE id = ? AND user_id = ?', [orderId, userId]);
        const order = rows[0];
        if (!order)
            return res.status(404).json({ error: '订单不存在' });
...
router.post('/:id/confirm-paid', authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        const orderId = parseInt(req.params.id, 10);
        if (isNaN(orderId))
            return res.status(400).json({ error: '无效订单' });
        const [rows] = await pool.execute('SELECT id, user_id, status FROM orders WHERE id = ? AND user_id = ?', [orderId, userId]);
        const order = rows[0];
        if (!order)
            return res.status(404).json({ error: '订单不存在' });
        if (order.status !== 'pending')
            return res.json({ ok: true, status: order.status });
        const [items] = await pool.execute('SELECT oi.product_id, oi.quantity, oi.price, p.popularity_boost, p.type FROM order_items oi JOIN products p ON oi.product_id = p.id WHERE oi.order_id = ?', [orderId]);
        const productList = items;
        let totalBoost = 0;
        for (const it of productList) {
            if (it.type === 'credit_restore') {
                for (let i = 0; i < it.quantity; i++) {
                    await pool.execute('INSERT INTO credit_purchases (user_id, amount, credit_restored) VALUES (?, ?, 1)', [order.user_id, parseFloat(it.price)]);
                ...
                if (it.type !== 'boost')
                    totalBoost += (it.popularity_boost || 0) * it.quantity;
            }
        }
