...
router.get('/', optionalAuthMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    if (city) { sql += ' AND (e.city = ? OR (e.city IS NULL AND e.location LIKE ?))'; params.push(city, `%${city}%`); }
    if (district) { sql += ' AND (e.district = ? OR (e.district IS NULL AND e.location LIKE ?))'; params.push(district, `%${district}%`); }
...
router.post('/', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    if (inserted && (cty || prov || dist)) {
      try {
        await pool.execute(
          'UPDATE events SET province = COALESCE(province, ?), city = COALESCE(city, ?), district = COALESCE(district, ?) WHERE creator_id = ? ORDER BY id DESC LIMIT 1',
          [prov || null, cty || null, dist || null, req.userId!]
        );
      } catch { /* 列可能不存在，忽略 */ }
    }
    const [rows] = await pool.execute(
