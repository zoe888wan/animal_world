...
router.get('/me', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    try {
      const [cr] = await pool.execute('SELECT credit_score FROM users WHERE id = ?', [req.userId]);
    ...
    try {
      const [cRows] = await pool.execute('SELECT COALESCE(coins, 0) as coins FROM users WHERE id = ?', [req.userId]);
...
router.post('/me/accessories/:id/equip', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    if (!up || up.user_id !== req.userId) return res.status(404).json({ error: '饰品不存在' });
    ...
    if (!pet || pet.owner_id !== req.userId) return res.status(403).json({ error: '只能给自己的宠物佩戴' });
...
router.post('/me/pet-interact', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    await pool.execute(
      ...
      [newCoins, today, count + (earned > 0 ? 1 : 0), req.userId]
    );
...
router.get('/me/credit-restore-info', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const [uRows] = await pool.execute('SELECT credit_score FROM users WHERE id = ?', [req.userId]);
...
router.put('/me', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    if (!hasChanges) {
      const [rows] = await pool.execute('SELECT id, username, email, avatar_url, nickname, bio, credit_score FROM users WHERE id = ?', [req.userId]);
    ...
    await pool.execute(
      ...
      [nick ?? null, bioVal ?? null, avatarVal ?? null, req.userId]
    );
    await pool.execute(
